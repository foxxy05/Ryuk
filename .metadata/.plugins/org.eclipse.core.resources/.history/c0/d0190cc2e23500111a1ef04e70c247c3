...
//} RampMotor;
//
//RampMotor RM1 = {0}, RM2 = {0}, RM3 = {0};

//#define RAMP_MAX_PWM  150.0f   // effective max after 1.5x multiply in find_wheel_vel
//
//float ramp_accel   = 0.6f;    // how fast velocity builds — tune this first
//float ramp_max_vel = 6.0f;    // caps rate of PWM change per 10ms cycle
//float ramp_damping = 0.95f;   // kills oscillation at target — lower = more damping
...
//}rampParameters;
//
//rampParameters Ramp = {150.0f, 0.64f, 6.0f, 0.95f};
/* USER CODE END PV */
...
// WORKING TURRET LOGIC
//void TurretMotor() {
//	if (right) {
//		RotateCytron(&Turret, 75);
//	} else if (left) {
//		RotateCytron(&Turret, -75);
//	} else {
//		StopCytron(&Turret);
...
////NEW_TURRET
#define turretRamp_UP    5
#define turretRamp_DOWN  15
...
//		ODrivePlace.flag = !ODrivePlace.flag;
//
//		if (ODrivePlace.flag) {
//			//ODrive_goTo(OdriveS1 *dev, float position, float velocity)
...
//void velocityRampUpdate(RampMotor *m) {
//    float error = m->targetPWM - m->currentPWM;
//
//    // Magnitude-based smoothstep — symmetric for + and - directions
//    float norm = fabs(error) / Ramp.MaxPWM;
//    if (norm > 1.0f) norm = 1.0f;                         // clamp — critical
//    float shaped = norm * norm * (3.0f - 2.0f * norm);    // smoothstep [0,1]
//    float direction = (error > 0.0f) ? 1.0f : -1.0f;
//
//    m->velocity += Ramp.Acceleration * shaped * direction; // was: ramp_accel
//
//    // Clamp velocity
//    if (m->velocity >  Ramp.MaxVelocity) m->velocity =  Ramp.MaxVelocity;  // was: m->ma
//    if (m->velocity < -Ramp.MaxVelocity) m->velocity = -Ramp.MaxVelocity;
//
//    // Damping — prevents overshooting target
//    m->velocity *= Ramp.Damping;                           // was: ramp_damping
//
//    m->currentPWM += m->velocity;
//
//    // Snap to target when close enough — prevents infinite crawl
//    if (fabs(error) < 1.5f) {
...
//}
//
//void updateAllRamps(void) {
//    velocityRampUpdate(&RM1);
//    velocityRampUpdate(&RM2);
//    velocityRampUpdate(&RM3);
//}

// manual control
//#define RAMP_STEP 18   // start here, go up to 20 if still feels slow
//
//float ramp1 = 0.0f, ramp2 = 0.0f, ramp3 = 0.0f;
//
//float linear_ramp(float current, float target) {
//    float error = target - current;
//    if (fabs(error) <= RAMP_STEP) return target;
...
//  ODrive_clearErrors(&M1);
	...
	InitCytron(&m1, &htim11, TIM_CHANNEL_1, GPIOF, GPIO_PIN_6, 1); // DIR PF6   PWM PF7    L3
...
//		RM1.targetPWM = (float)pwm1;
//		RM2.targetPWM = (float)pwm2;
//		RM3.targetPWM = (float)pwm3;

		updateAllRamps(isZeroInput);
		rotate_motor((int) ramp1, (int) ramp2, (int) ramp3);
