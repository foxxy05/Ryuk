...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define KFS_CONVEYOR_SPEED 	50
#define KFS_MOTOR_FREQ		1000
#define KFS_APB_BUS		    2
#define KFS_LPWM 			TIM_CHANNEL_2
#define KFS_RPWM 			TIM_CHANNEL_3


#define KFS_OPEN_ANGLE 		90
#define KFS_CLOSE_ANGLE     0
#define KFS_SERVO 			12
#define KFS_PCA_FREQ 		50
...
void get_value() {
	...
	if (I2C_Status == HAL_OK) {
		...
		Joystick.Omega 		= data[6];
		Joystick.RY 		= data[5];
		Joystick.RX 		= data[4];
		Joystick.LY 		= data[3];
		Joystick.LX 		= data[2];

		if (abs(Joystick.LX) < DEADZONE) {
			Joystick.LX = 0;
		}
		if (abs(Joystick.LY) < DEADZONE) {
			Joystick.LY = 0;
		}
		if (abs(Joystick.Omega) < DEADZONE) {
			Joystick.Omega = 0;
		}
...
void KFS_Motion(){
	if(PS4.L1){
		RotateMotor(&KFS_MOTOR,   KFS_CONVEYOR_SPEED);
...
//		HAL_UART_Transmit(&huart3, (uint8_t*) msg, sizeof(msg), 100);
	...
	if(PS4.L3){
		RotateServo(&KFS_PCA, KFS_SERVO, KFS_OPEN_ANGLE);
//		sprintf(msg, "L3 Pushed!\r\n");
//		HAL_UART_Transmit(&huart3, (uint8_t*) msg, sizeof(msg), 100);
	}
	if(PS4.R3){
		RotateServo(&KFS_PCA, KFS_SERVO, KFS_CLOSE_ANGLE);
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
//	spitDefault(&huart3);
	InitBTS(&KFS_MOTOR, &htim8, KFS_LPWM, &htim8, KFS_RPWM, KFS_APB_BUS, KFS_MOTOR_FREQ);
//	InitPCA(&KFS_PCA, &hi2c3, KFS_PCA_FREQ);
  /* USER CODE END 2 */
